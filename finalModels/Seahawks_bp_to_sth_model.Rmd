...
bp_to_sth_model <- suppressWarnings(train(CONVERSION ~ . + , 
                         data = trainSet, 
                         method = 'glm',
                         family = "binomial",
                         trControl = myCtrl))
...
predicted_classes <- ifelse(predictions[, 2] > 0.3, 1, 0)
...
baseline_customer <- data.frame(
  email_send = (200 - mean(numeric_vars$email_send)) / sd(numeric_vars$email_send),
  ...
  primary_purchases = (1 - mean(numeric_vars$primary_purchases)) / sd(numeric_vars$primary_purchases),
